...
Use `ops-weekly` and `meeting-actions` first. After every real run, record time saved, errors, rework, and adoption with `npm run evaluate`. Stabilize only workflows that are used three or more times per week or clearly save recurring review time.
